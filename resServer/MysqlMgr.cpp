...
bool MysqlMgr::insertFileInfo(std::string& filemarking, std::string& filename, int64_t& filesize, int& uid, std::string& filepath, std::string& tip)
{
	return _dao.insertFileInfo(filemarking,filename,filesize ,uid,filepath,tip);
}

bool MysqlMgr::getFilePathFromMarking(const std::string& marking, fileType& filepath)
{
	return _dao.getFilePathFromMarking(marking, filepath);
}
