...
bool MysqlDao::insertFileInfo(std::string& filemarking, std::string& filename, int64_t& filesize, int& uid, std::string& filepath, std::string& tip)
{
	auto con = _sqlpool->getConnection();
	if (con == nullptr) return false;
	...
	try {
		/*std::unique_ptr<sql::PreparedStatement> ps(
			con->_con->prepareStatement(
				...
				"(marking, filename, filesize, useruid, filepath, description) "
				"VALUES (?, ?, ?, ?, ?, ?)"
			)
		);*/
		std::string sql = "INSERT INTO file_message " 
			"(marking, filename, filesize, useruid, filepath, description) " 
			"VALUES (?, ?, ?, ?, ?, ?)";
		std::unique_ptr<sql::PreparedStatement> ps(con->_con->prepareStatement(sql));


		ps->setString(1, filemarking);         // 假设 filemarking 本身是 UTF-8
		ps->setString(2, filename);       // 转换后插入
		ps->setInt64(3, filesize);
		ps->setInt(4, uid);
		ps->setString(5, filepath);       // 转换后插入
		ps->setString(6, tip);            // 转换后插入

		int affected = ps->executeUpdate();  // 用 executeUpdate 获取影响行数
...
bool MysqlDao::getFilePathFromMarking(const std::string& marking, fileType& fileInfo)
{
	auto con = _sqlpool->getConnection();
	if (con == nullptr) return false;
	...
	try {
		std::unique_ptr<sql::PreparedStatement> ps(
			con->_con->prepareStatement(
				"SELECT filepath, filename, filesize FROM file_message WHERE marking = ?"
			)
		);
		ps->setString(1, marking);

		std::unique_ptr<sql::ResultSet> rs(ps->executeQuery());
		if (rs->next()) {
			fileInfo.filepath = rs->getString("filepath");
			fileInfo.filename = rs->getString("filename");
			fileInfo.filesize = rs->getInt64("filesize");
			fileInfo.filemarking = marking;
			return true;
		}
		else {
			return false; // 未找到记录
