...
private:
	...
	std::queue < std::unique_ptr<StatusServer::Stub>> _connections; //Stub（存根）是客户端调用远程服务的本地代理对象

public:
	StatusConPool(std::size_t poolsize, std::string host, std::string port);
	~StatusConPool();
	//从池中获取一个stub 连接
	std::unique_ptr<StatusServer::Stub> getConnection();
	//返还一个stub连接
	void returnConnection(std::unique_ptr<StatusServer::Stub>context);
	void close();
};
