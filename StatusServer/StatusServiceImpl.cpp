...
#include "ConfigMgr.h"
#include "const.h"
#include <boost/uuid/uuid.hpp>             // 基本 UUID 类型
#include <boost/uuid/uuid_generators.hpp>  // 生成器（如 random_generator）
#include <boost/uuid/uuid_io.hpp>          // to_string 等 I/O 支持
#include "RedisMgr.h"
#include "Defer.h"
...
//回应 grpc 客户端的 GetChatServer 函数请求实现
Status StatusServiceImpl::GetChatServer(ServerContext* context, const GetChatServerReq* request, GetChatServerRsp* reply)
{
	std::string prefix("status server has received :  ");
	const auto& server = getChatServer();                     //返回压力最小的聊天服务器
	reply->set_host(server.host);
	reply->set_port(server.port);
	reply->set_error(ErrorCodes::SUCCESS);
	reply->set_token(generate_unique_string());
	insertToken(request->uid(), reply->token());    //将uid + token 存储起来
	return Status::OK;
}
